...
Usage: parce_reciprocal_blast_data.pl --query_vs_target <file> --target_vs_query <file>
...
while(my $blast_query=<QUERY_BLAST>){
	...
	$target=~/([A-Z0-9]+)\|([A-Z0-9]+)\|*(\S+)*/;
	my $target_gene_id=$1; 
	my $target_transcript_id=$2;
	my $target_gene_name=$3;
	...
	#print "target_gene_id: $target_gene_id target_transcript_id: $target_transcript_id target_gene_name $target_gene_name\n";
	...
	$blast_info_query{$gene}.="$query\t$target_gene_name\t$target_gene_id\t$target_transcript_id\t$target_start\t$target_end\t$perc_id\t$al_length\t$evalue\t$bit_score";
...
print "gene\ttranscript_used\ttarget_name\ttarget_id\ttarget_transcript\ttarget_start\ttarget_end\tperc_id\tal_length\tevalue\tscore\tn_hits_same_target\t";
print "reciprocity\treciprocal_gene\n";
