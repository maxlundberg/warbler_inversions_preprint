...
my ($gene_info_file,$blast_summary_file,$kraken_intersect_file,$swissprot_blast,$swissprot_info_file,$interpro_summary,$help);
...
Usage: summarize_annotation_data.pl --gene_info_file <file> --blast_summary_file <file> --kraken_intersect_file <file>
...
if(!$gene_info_file || !$blast_summary_file || !$kraken_intersect_file || !$swissprot_blast || !$swissprot_info_file || !$interpro_summary){
...
$new_column_heads.="blast_gene_name\tblast_gene_id\tblast_gene_transcript\ttarget_start\ttarget_end\tperc_id\talign\tevalue\tbitscore\tn_genes_same_blast\treciprocity\ttarget_hit\ttarget_perc\ttarget_aln\t";
...
while(my $gene_info=<GENE_INFO>){
	...
	if(exists($blast_summary{$gene})){
		...
		@recip_blast_info=("NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA","NA")
		}

	my ($blast_query_transcript,$blast_target_gene_name,$blast_target_gene_id,$blast_target_transcript,$blast_target_start,$blast_target_end,
...
#	print "\n$swissprot_gene,$swissprot_perc,$swissprot_length,$swissprot_evalue,$swissport_score\n";
	...
	my ($gene_annotation,$annotation_source);
		...
		if($kraken_gene_id ne "no_overlap"){
			$gene_annotation=$kraken_gene_name;
			$annotation_source="kraken";
			}
		else{
			if($blast_target_gene_name ne "NA"){
				...
				if($swissprot_gene ne "NA"){
					...
					$gene_annotation="uncharacterized";
	...
	my $gene_name="ww_gene_${gene_count}_${gene_annotation}";
	...
	print "$blast_target_gene_name\t$blast_target_gene_id\t$blast_target_transcript\t$blast_target_start\t$blast_target_end\t$perc_id\t$aln_length\t$evalue\t$score\t";
	print "$n_hits_same_target\t$reciprocity\t$reciprocal_gene\t$reciprocal_perc\t$reciprocal_aln_length\t";
